...
def _set_global_determinism(seed: int = 2025):
    random.seed(seed)
    np.random.seed(seed)
...
LOCAL_QWEN_ALIASES = {"qwen-7b", "qwen-14b", "qwen-30b"}
...
def main():
    ...
    parser.add_argument(
        "--two_stage_window",
        action="store_true",
        help="Use 2-stage judging for all_at_once: (1) global prediction, then (2) window-based final judge around the predicted step.",
    )
    parser.add_argument(
        "--method",
        type=str,
    ...
    two_stage_suffix = "_2stage" if getattr(args, "two_stage_window", False) and args.method == "all_at_once" else ""
