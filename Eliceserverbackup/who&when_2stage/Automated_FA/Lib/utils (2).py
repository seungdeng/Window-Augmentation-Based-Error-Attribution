...
def all_at_once_with_window(client: AzureOpenAI, directory_path: str, is_handcrafted: bool, model: str, max_tokens: int):
    ...
    for json_file in tqdm(json_files):
        ...
        if isinstance(final_step, int) and 0 <= final_step < L:
            final_agent = chat_history[final_step].get(index_agent, agent_1 or "Unknown Agent")
        else:
            final_agent = agent_1 or "Unknown Agent"
...
def step_by_step_with_window(client: AzureOpenAI, directory_path: str, is_handcrafted: bool, model: str, max_tokens: int):
    """
